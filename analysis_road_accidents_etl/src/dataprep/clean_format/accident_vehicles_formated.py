...
def accident_vehicles_formated():
    return (
        ...
        .withColumn(
            "initial_point_of_impact_group",
            F.when(F.col("initial_point_of_impact").between(-1, 10), "Manoeuvre principale avant l’accident")
             .when(F.col("initial_point_of_impact").between(11, 12), "Changeant de file")
             .when(F.col("initial_point_of_impact").between(13, 14), "Déporté")
             .when(F.col("initial_point_of_impact").between(15, 16), "Tournant")
             .when(F.col("initial_point_of_impact").between(17, 18), "Dépassant")
             .when(F.col("initial_point_of_impact").between(19, 26), "Divers")
             .otherwise("")
        )
        .withColumn(
